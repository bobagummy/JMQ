...
public class LevelGenerator : MonoBehaviour {
    ...
    void Start() {
        ...
        //map
        else if (levelType == LevelType.Map) {
    ...
    private void GenerateWin() {
        ...
        //knights
        SpawnObjectsWithTag("Knight");

        //collectables
        SpawnObjectsWithTag("Collectable");

        //retrieve all hero and knight characters
        List<GameObject> allChars = new List<GameObject>();
        allChars.AddRange(GameObject.FindGameObjectsWithTag("Hero"));
        allChars.AddRange(GameObject.FindGameObjectsWithTag("Knight"));

        //iterate through characters
        foreach (GameObject aChar in allChars) {

            //retrieve collider
            Collider2D collider = aChar.GetComponent<Collider2D>();

            //if valid
            if (collider) {

                //disable collisions
                aChar.GetComponent<Collider2D>().enabled = false;
            }
        }

        //animation
        AnimateObjectsWithTag("Hero");
        AnimateObjectsWithTag("Knight");
    }
    ...
    private void GenerateDungeon() {
        ...
        if (isHeroFound == false) {
            ...
            //movement
            StartCoroutine(MoveObjectsWithTag("Drake", true, 1.0f));

            //animation
            //drakes
            AnimateObjectsWithTag("Drake");
        }
        ...
        //hero found, but dragon not yet defeated
            ...
            //movement
            StartCoroutine(MoveObjectsWithTag("Dragon", true, 1.0f));

            //animation
            //dragon
            AnimateObjectsWithTag("Dragon");
        }
        ...
        //audio
        //if dungeon music is not already playing
		if (AudioManager.Instance.bgmSource.clip != 
			AudioManager.Instance.bgmDungeon) {
    ...
    //modify movement on objects with tag
    public IEnumerator MoveObjectsWithTag(string theTag, bool theIsEnabled, float theDelay = 0.0f) {
        ...
        for (int i = 0; i < objects.Length; i++) {
            ...
            //retrieve movement script
            AIMove move = objects[i].GetComponent<AIMove>();

            //update movement
            //enabled
            if (move == true && theIsEnabled == true) {
            ...
            //disabled
            else if (move == true && theIsEnabled == false) {
    ...
    //animate objects associated with a specific tag
    private void AnimateObjectsWithTag(string theTag) {
        ...
        for (int i = 0; i < objects.Length; i++) {
            ...
            //retrieve animator
            CharacterAnimator animator = objects[i].GetComponent<CharacterAnimator>();
            ...
            if (animator) {
                ...
                switch (theTag) {
                    ...
                    case "Knight":
                        ...
                        //set up animation
                        animator.SetUpAnimationWithName("ActDown", 3.0f);
                    ...
                    case "Hero":
                        ...
                        //set up animation
                        animator.SetUpAnimationWithName("ActDown", 6.0f);
                    ...
                    case "Drake":
                        ...
                        //set up animation
                        animator.SetUpAnimationWithName("Fly", 3.0f);
                    ...
                    //default
                    default:
                        Debug.Log("[LevelGenerator] Animation tag not recognized");
                        break;
                }
