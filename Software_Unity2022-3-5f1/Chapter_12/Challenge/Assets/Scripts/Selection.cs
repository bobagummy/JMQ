...
public class Selection : MonoBehaviour {
    ...
    //create a selection box with given choices
    public void CreateSelection(List<string> theText) {
        ...
        for (int i = 0; i < theText.Count; i++) {
            ...
            //retrieve text
            Text txt = txtObjects[i].GetComponent<Text>();
    ...
    private void ClearText() {
        ...
        //iterate through text objects
        foreach (GameObject aTxtObj in txtObjects) {
    ...
    private void ResetIndicator() {
		...
		//update y to match top choice
        newPos.y = txtObjects[0].transform.position.y;
    ...
    public IEnumerator StartSelection(int theNumChoices, SelectedChoice theChoice) {

        //whether selection is dismissed
        ...
        while (isDismissed == false) {
            ...
            yield return StartCoroutine(
                WaitForChoice(theNumChoices, value => theChoice(value))
                );
    ...
    public IEnumerator WaitForChoice(int theNumChoices, SelectedChoice theChoice) {
        ...
        while (isChoiceSelected == false) {
            ...
            //update y to match current choice
            newPos.y = txtObjects[currentChoice].transform.position.y;
